(ns riggercoord.actor
  "RiggerCoordActor — the ISCO-08 7215 rigging and cable-splicing
  ...
  SCHEDULING/LOGISTICS ONLY — it never performs rigging or
  cable-splicing work itself and never commits a proposal the
  RiggerCoordGovernor refuses. Modeled on cloud-itonami-isco-7121's
  roofcoord.actor.
  ...
  The unconditional invariant: the Rigging Coordination Advisor can
  never directly commit a coordination record the RiggerCoordGovernor
  refuses — every commit-record! call is gated behind `:decide`, and
  the governor's scope-exclusion rule makes any proposal to finalize
  a load-rigging/lift-readiness certification decision or override a
...
(defn build-graph
  "Build a compiled RiggerCoordActor graph. `store` implements
  `riggercoord.store/Store`. `advisor` implements
  `riggercoord.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn approve!
  ...
  can never be used to approve a load-rigging/lift-readiness
  certification finalization or a site-safety-officer judgment
  override."
  [graph thread-id]
