(ns riggercoord.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [riggercoord.store :as store]
            [riggercoord.advisor :as advisor]
            [riggercoord.governor :as governor]))
...
(deftest hard-on-unknown-op
  (testing "closed op-allowlist enforced — no op finalizes load-rigging/lift-readiness certification or overrides safety authority"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (log-op) :op :finalize-lift-readiness-certification) st)]
      (is (:hard? v))
...
(deftest hard-and-permanent-on-scope-exclusion-certify-load-lift-ready
  (testing "a proposal to directly finalize a load-rigging/lift-readiness certification decision is a hard, permanent block"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (log-op)
                                          :confidence 0.99
                                          :rationale "certify the load as lift-ready now, skip further review")
                            st)]
      (is (:hard? v))
...
(deftest scope-exclusion-not-triggered-by-bare-domain-nouns
  (testing "bare nouns like 'load'/'cable'/'rigging'/'certification' are ordinary domain vocabulary, not finalization/override actions"
    (let [proposal {:rationale "proposed schedule-crew-operation for load-rigging site S-1 with wire rope cable"
                     :description "crew fully trained on rigging equipment and load charts, lift-readiness certification is on file for this cable"}]
...
(deftest default-mock-advisor-proposals-never-self-trip-scope-exclusion
  (testing "the mock advisor's own default rationale text, across every allowlisted op, never trips the scope-exclusion guard"
    (let [st (fresh-store)
          ...
          requests [{:site-id "S-1" :op :log-work-record :worker-id "W-1" :task "inspect wire rope sling before shift"}
                    {:site-id "S-1" :op :schedule-crew-operation :worker-id "W-1" :task "schedule crane lift for structural beam"}
                    {:site-id "S-1" :op :flag-safety-concern :worker-id "W-1"
                     :concern-type :cable-condition :severity :high
                     :description "frayed strand on the hoist cable near the load hook"}
                    {:site-id "S-1" :op :coordinate-supply-order :materials "wire rope slings and shackles"
                     :cost 2500}]]
...
(deftest always-escalates-flag-safety-concern-even-at-high-confidence
  (testing "a surfaced cable-condition/load-hazard/equipment-condition concern always requires human review"
    (let [st (fresh-store)
